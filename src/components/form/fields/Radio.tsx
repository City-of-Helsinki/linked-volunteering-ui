...
const RadioField: React.FC<RadioFieldProps> = ({ id, label, required, error, touched, text, ...rest }) => {
  const { formatMessage } = useIntl();

  return (
    <FormGroup check>
      {label && <Label for={id}>{formatMessage({ id: label })}</Label>}
      <Input required={required} id={id} type='radio' invalid={!!error && touched} {...rest} />
